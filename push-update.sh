...
updates_dir=/data/everest_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have everest-VERSION-DATE-TYPE-* as format"
...
# Assume everest-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.everest.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.everest.updater/databases/updates.db" \
